...
public class DAOArchivioJson implements IDAOArchivio {
    ...
    public Archivio carica(InputStream inputStream) throws DAOException {
        try {
            ...
            logger.debug("Caricato l'archivio aggiornato in data " + archivio.getDataAggiornamento() + " contenente " + archivio.getListaComuni().size() + " comuni");
            return archivio;
        } catch (Exception e) {
            throw new DAOException("Impossibile caricare il file json " + e.getLocalizedMessage());
        }
    }
